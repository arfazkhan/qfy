        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '0.85rem', color: 'rgba(255,255,255,0.4)', fontWeight: 600 }}>
              ...
              <span style={{ color: 'var(--gold-primary)' }}>{business.business_nature?.toUpperCase() || 'TRADING (LLC)'}</span>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '40px', alignItems: 'center' }}>
            ...
            <div style={{ borderLeft: '1px solid rgba(255,255,255,0.1)', paddingLeft: '40px' }}>
              ...
              <p style={{ fontSize: '0.9rem', fontWeight: 700, color: '#fff' }}>{new Date().toLocaleString()}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          ...
          <div className="luxury-card" style={{ padding: '24px' }}>
            ...
            <div className="info-table">
              {[
                ...
                ['CR Status', <span style={{ color: '#10b981', fontWeight: 800 }}>ACTIVE</span>]
              ].map(([label, value], i) => (
          ...
          <div className="luxury-card" style={{ padding: '24px' }}>
            ...
            <div style={{ 
              ...
              {(business.activities || []).map((item: any, i: number) => (
                <div key={i} style={{ display: 'flex', gap: '16px', position: 'relative' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    ...
                    {i !== ((business.activities || []).slice(0, 4).length - 1) && (
                      <div style={{ flex: 1, width: '1px', background: 'rgba(255,255,255,0.05)', margin: '4px 0' }}></div>
                    )}
                  </div>
                  <div style={{ flex: 1, paddingBottom: i === 3 ? '0' : '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }} ref={peopleSectionRef}>
          <div className="luxury-card" style={{ padding: '24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {[
                { label: 'BUSINESS OWNER', person: business.owner, role: 'OWNER' },
                { label: 'MANAGER INCHARGE', person: business.manager, role: 'MANAGER' },
                { label: 'AUTHORIZED PERSON', person: business.authorized_person, role: 'AUTH' }
              ].map(({ label, person, role }) => {
                const expiryDate = person?.expiry_date ? new Date(person.expiry_date) : null;
                const today = new Date();
                const isExpired = expiryDate && expiryDate.getTime() < today.getTime();
                const diffDays = expiryDate ? Math.ceil((expiryDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)) : 0;
                const isGracePeriod = isExpired && Math.abs(diffDays) <= 30;
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                        <div style={{
                          ...
                          {role === 'AUTH' ? <ShieldCheck size={20} color="#3b82f6" /> : <User size={20} color="var(--gold-primary)" />}
                        </div>
                        <div>
                          <div style={{ fontSize: '0.6rem', fontWeight: 900, color: role === 'AUTH' ? '#3b82f6' : 'var(--gold-primary)', letterSpacing: '0.5px', marginBottom: '4px' }}>{label}</div>
                          <h4 style={{ fontSize: '1rem', fontWeight: 700, color: '#fff', margin: '0' }}>{person?.name || 'Not Assigned'}</h4>
                          <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', margin: '2px 0 0' }}>QID: {person?.qid_number || '—'}</p>
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px' }}>
                        {expiryDate && (
                          <div style={{
                            ...
                            color: isExpired ? '#ef4444' : (isWarning ? '#f59e0b' : 'rgba(255,255,255,0.3)')
                          }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                          {(isExpired || isGracePeriod || !expiryDate) ? (
                            <button
                              className="btn-luxury"
                              ...
                              onClick={() => { setSelectedPerson(person); setShowScanModal(true); }}
                            >
                              Update ID
                            </button>
                          ) : (
      ...
      {showScanModal && (
        <div style={{
          ...
          <div className="luxury-card" style={{ width: '95%', height: '90%', padding: '20px', position: 'relative', overflow: 'hidden' }}>
            ...
            <iframe
              src={`/scan?embedded=true&link_business_id=${business.id}&link_role=${selectedPerson?.role || 'STAFF'}${selectedPerson ? `&qid=${selectedPerson.qid_number}&re_scan=true` : ''}`}
              ...
              <div style={{ display: 'flex', gap: '12px' }}>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    // Pass business ID and role via custom state or search params if needed
                    // For now, scan page will handle it if we set link params
                  }}
                >
                  <Plus size={18} />
                  Add New Person
                ...
                <tbody>
                  {peopleList.length === 0 ? (
                    ...
                    peopleList.map((person, idx) => {
                      ...
                      let statusColor = 'rgba(255,255,255,0.3)'; // Non-prominent for ACTIVE
                      let statusLabel = 'ACTIVE';

                      if (diffDays < 0) {
                        const graceDays = Math.abs(diffDays);
                        if (graceDays <= 30) {
                          statusColor = '#ef4444'; // Red as per user request
                        ...
                        <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                          ...
                          <td style={{ padding: '20px 24px' }}>
                            <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                              ...
                              <button className="action-btn-dim"><Eye size={14} /></button>
                            </div>
                          </td>
                        </tr>
                      );
